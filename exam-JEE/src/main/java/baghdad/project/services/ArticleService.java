...
public class ArticleService  implements IDao<Article>{
    ...
    public List<Article> findByDateArticles(@RequestParam @DateTimeFormat(pattern = "yyyy-MM-dd") Date dateDebut,@RequestParam @DateTimeFormat(pattern = "yyyy-MM-dd") Date dateFin) {
        return articleRepository.findByDateArticles(dateDebut, dateFin);
    }
